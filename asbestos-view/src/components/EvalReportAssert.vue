<template>
    <div>
<!--        assert pass/fail-->
        <div v-bind:class="{pass : isAssertPass, fail: !isAssertPass}"  @click="toggleAssertMessageDisplay()">
...
<!--        events listed par assert-->
        <div v-if="displayAssertMessage" class="event-part">
            <div v-for="(eventId, eventi) in eventIds" v-bind:class="{'pass': isEventPass(eventId), fail: !isEventPass(eventId)}">
                <div>
                    <eval-report-event :event-id="eventId" :test-report="eventResult[eventId]"></eval-report-event>
                </div>
            </div>
<!--            <pre>{{ message }}</pre>-->
        </div>
