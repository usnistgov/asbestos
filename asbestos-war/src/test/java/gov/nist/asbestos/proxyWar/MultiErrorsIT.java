...
public class MultiErrorsIT {
    ...
    @Test
    void multiErrorsTest() throws URISyntaxException {
        TestEngine engine = Utility.run(base, "/engine/multiErrors/TestScript.xml");
        ...
        // assert (action 2) in test 0 runs because of multiErrors extension
        assertEquals(TestReport.TestReportActionResult.FAIL, engine.getTestReport().getTest().get(0).getAction().get(2).getAssert().getResult());

        // test 1 does run because it is a different test
