...
class CreateChannelIT {
    ...
    void createAChannel() throws URISyntaxException, IOException {
        ...
        poster.postJson(new URI("http://localhost:8877/proxy/prox"), ChannelConfigFactory.convert(channelConfig));
        ...
        getter.getJson(new URI("http://localhost:8877/proxy/prox/default__test"));
    ...
    void deleteAndRecreateChannel() throws URISyntaxException, IOException {
        ...
        deleter.run(new URI("http://localhost:8877/proxy/prox/default__test"));
        ...
        HttpPost poster = new HttpPost();
        poster.postJson(new URI("http://localhost:8877/proxy/prox"), json);
    ...
    void createMultipleChannels() throws URISyntaxException, IOException {
        ...
        deleter.run(new URI("http://localhost:8877/proxy/prox/default__test1"));
        assertNotEquals(500, deleter.getStatus());

        deleter = new HttpDelete();
        deleter.run(new URI("http://localhost:8877/proxy/prox/default__test2"));
        assertNotEquals(500, deleter.getStatus());

        // create - must return 201 (didn't exist)
        poster = new HttpPost();
        poster.postJson(new URI("http://localhost:8877/proxy/prox"), channelConfig1);
        assertEquals(201, poster.getStatus());

        // create - must return 201 (didn't exist)
        poster = new HttpPost();
        poster.postJson(new URI("http://localhost:8877/proxy/prox"), channelConfig2);
        assertEquals(201, poster.getStatus());
