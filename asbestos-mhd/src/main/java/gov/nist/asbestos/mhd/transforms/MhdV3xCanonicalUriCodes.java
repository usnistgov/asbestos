...
public class MhdV3xCanonicalUriCodes implements MhdCanonicalUriCodeInterface {
    ...
    @Override
    public Map<CanonicalUriCodeEnum, String> getUriCodes(UriCodeTypeEnum uriCodeTypeEnum) {
        return canonicalUriCodeEnumStringMap.entrySet().stream()
                .filter(e -> uriCodeTypeEnum.getType().equals(e.getKey().getType())).collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
    }

    @Override
    public Map<CanonicalUriCodeEnum, String> getAllUriCodes() {
